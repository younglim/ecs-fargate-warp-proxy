...
RUN set -eux; \
    case "${TARGETARCH}" in \
      amd64|arm64) BIN_ARCH="${TARGETARCH}" ;; \
      *) echo "Unsupported TARGETARCH: ${TARGETARCH}" >&2; exit 1 ;; \
    esac; \
    curl -fsSL "https://github.com/ViRb3/wgcf/releases/download/v2.2.30/wgcf_2.2.30_linux_${BIN_ARCH}" -o /usr/local/bin/wgcf && \
...
RUN set -eux; \
    ...
    curl -fsSL "https://github.com/octeep/wireproxy/releases/download/v1.1.2/wireproxy_linux_${BIN_ARCH}.tar.gz" -o /tmp/wireproxy.tar.gz && \
